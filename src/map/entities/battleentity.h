...
enum SUBSKILLTYPE
{
	...
	// BST jug pets must be added here and match the pet_list.sql database
	// lua_baseentity.cpp hasValidJugPetItem() function checks this list for valid jug pets
    SUBSKILL_SHEEP      = 21,
    SUBSKILL_HARE       = 22,
    SUBSKILL_CRAB       = 23,
    SUBSKILL_CARRIE     = 24,
...
public:
    ...
    uint16          DEF();
    uint16          ATT();
    uint16			ACC(uint8 attackNumber, int16 offsetAccuracy);
    uint16          EVA();
