...
CEntityUpdatePacket::CEntityUpdatePacket(CBaseEntity* PEntity, ENTITYUPDATE type, uint8 updatemask)
{
    ...
    if (PEntity->allegiance == ALLEGIANCETYPE::ALLEGIANCE_PLAYER)// && PEntity->status == STATUSTYPE::UPDATE)
    {
        ref<uint8>(0x20) = static_cast<uint8>(STATUSTYPE::STATUS_NORMAL);
    }
    else
    {
    ...
    switch (PEntity->objtype)
    {
        ...
        case TYPE_TRUST:
        {
            ...
            if (updatemask & UPDATE_HP)
            {
                ...
                ref<uint8>(0x28) |= PMob->status == STATUSTYPE::STATUS_NORMAL && PMob->objtype == TYPE_MOB ? 0x40 : 0; // Make the entity triggerable if a mob and normal status
                ref<uint8>(0x29) = static_cast<uint8>(PEntity->allegiance);
                ref<uint8>(0x2B) = PEntity->namevis;
            }
