...
function onBattlefieldEnter(player, battlefield)
    player:delKeyItem(tpz.ki.PHANTOM_GEM_OF_ARROGANCE)
end
...
function onEventFinish(player, csid, option)
    if csid == 32001 then
        -- player:delKeyItem(tpz.ki.PHANTOM_GEM_OF_ARROGANCE)
        local killCounter = player:getCharVar("KillCounter_AAEV")
        player:setCharVar("KillCounter_AAEV", killCounter + 1)
        ...
        if
            player:getCharVar("KillCounter_AAEV") > 0 and
            not player:hasSpell(993)
        then
            player:addSpell(993, false, true)
        end
    end
end
