...
function onBattlefieldEnter(player, battlefield)
    player:delKeyItem(tpz.ki.PHANTOM_GEM_OF_COWARDICE)
end
...
function onEventFinish(player, csid, option)
    if csid == 32001 then
        -- player:delKeyItem(tpz.ki.PHANTOM_GEM_OF_COWARDICE)
        local killCounter = player:getCharVar("KillCounter_AATT")
        player:setCharVar("KillCounter_AATT", killCounter + 1)
        ...
        if
            player:getCharVar("KillCounter_AATT") > 0 and
            not player:hasSpell(995)
        then
            player:addSpell(995, false, true)
        end
    end
end
