...
function onTrigger(player, npc)
    ...
    player:startEvent(2132, 3407872, 3407928, 5505100, 4194372, 255, 0, 1, cruor)
end

function onEventUpdate(player, csid, option)
--	printf("Aby-Uleguerand veridical_conflux_01.cpp onEventUpdate CSID: [%i]  OPTION: [%i]\n", csid, option)
	local cruor = player:getCurrency("cruor")
	if (option == 131073 and cruor >= 56) then -- Veridical Conflux #02
		player:updateEvent(1, 0, 0, 4194372, 255, 0, 1, cruor) -- ?, ?, ?, flux id?, fluxesUnlocked, ?, ?, cruor
		player:delCurrency("cruor", 56)
	elseif (option == 196609 and cruor >= 60) then -- Veridical Conflux #03
		player:updateEvent(1, 0, 0, 4194372, 255, 0, 1, cruor)
