...
function onTrigger(player, npc)
	...
	if (player:hasKeyItem(tpz.ki.VOIDSTONE1)) then
		...
		if (npc:getID() == 17269271) then
			player:setLocalVar("VOIDWATCH_LOCATION", 1)
		elseif (npc:getID() == 17269272) then
			player:setLocalVar("VOIDWATCH_LOCATION", 2)
		elseif (npc:getID() == 17269273) then
...
function onEventFinish(player, csid, option)
	-- Spawn Goji (Jeuno T1 Voidwatch)
	if (option == 1) then
		if (player:getLocalVar("VOIDWATCH_LOCATION") == 1 and not GetMobByID(17269127):isSpawned()) then
			SpawnMob(17269127):updateClaim(player)
			player:delKeyItem(tpz.ki.VOIDSTONE1)
		elseif (player:getLocalVar("VOIDWATCH_LOCATION") == 2 and not GetMobByID(17269128):isSpawned()) then
			SpawnMob(17269128):updateClaim(player)
			player:delKeyItem(tpz.ki.VOIDSTONE1)
		elseif (player:getLocalVar("VOIDWATCH_LOCATION") == 3 and not GetMobByID(17269129):isSpawned()) then
			SpawnMob(17269129):updateClaim(player)
			player:delKeyItem(tpz.ki.VOIDSTONE1)
		end
	end
end