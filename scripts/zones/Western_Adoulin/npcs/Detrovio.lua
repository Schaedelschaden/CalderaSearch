...
function onTrade(player, npc, trade)
	if (player:getCharVar("KillCounter_Kirin") > 0 and player:getCharVar("KillCounter_JailOfLove") > 0) then
		...
		elseif (hasExistingAug == false and augStoneCategory > 12) then
			player:PrintToPlayer(string.format("Detrovio : Are you trying to make this process blow up in my face?! You have to have an existing augment for that to work."),tpz.msg.channel.NS_SAY)
