...
function onTrigger(player, npc)
	...
	local currentPortal = 0 -- Removes the portal the player is currently interacting with from the selection list
	local currentUnlocked = 1 -- Reverse logic. Locked = 1, Unlocked = 0
	
	if (player:getCharVar("EschaZitah_Portal1") == 1) then
		currentUnlocked = 0
	end
	...
	player:setCharVar("EschaZitah_Portal1", 1)
end
