...
function onEventFinish(player, csid, option)
    ...
    local ItemID = 0
    local Quantity = 1
--	printf("Aby-Grauberg Cruor_Prospector.cpp onEventFinish CSID: [%i]  OPTION: [%i]\n", csid, option)
	
	-- Item Exchanges - Page 1
    if option == 65537 then -- Unkai Kote
        Price = 5000
        ItemID = 12059
    elseif option == 131073 then -- Iga Tekko
        Price = 5000
        ItemID = 12060
    elseif option == 196609 then -- Lancer's Vambraces
        Price = 5000
        ItemID = 12061
    elseif option == 262145 then -- Caller's Bracers
        Price = 5000
        ItemID = 12062
    elseif option == 327681 then -- Mavi Bazubands
        Price = 5000
        ItemID = 12063
	elseif option == 393217 then -- Navarch's Gants
        Price = 5000
        ItemID = 12064
	elseif option == 458753 then -- Cirque Guanti
        Price = 5000
        ItemID = 12065
	elseif option == 524289 then -- Charis Bangles
        Price = 5000
        ItemID = 12066
	elseif option == 589825 then -- Savant's Bracers
        Price = 5000
        ItemID = 12067
	elseif option == 655361 then -- Utilis Shield
        Price = 5000
        ItemID = 16191
		
	-- Item Exchanges - Page 2
	elseif option == 720897 then -- Pyf Harp
        Price = 5000
        ItemID = 18573
	end
    ...
    if option == 17563649 then -- 1x Forbidden Key
        Price = 500
        ItemID = 2490
    elseif option == 84672513 then -- 5x Forbidden Key
        Price = 2500
        ItemID = 2490
        Quantity = 5
    elseif option == 168558593 then -- 10x Forbidden Key
        Price = 5000
        ItemID = 2490
        Quantity = 10
    elseif option == 504102913 then -- 30x Forbidden Key
        Price = 15000
        ItemID = 2490
        Quantity = 30
    elseif option == 839647233 then -- 50x Forbidden Key
        Price = 25000
        ItemID = 2490
        Quantity = 50
    elseif option == 917505 then -- Shadow Throne
        Price = 2000000
        ItemID = 6410
	...
	-- Key Item Exchanges
    elseif option == 65539 then -- Map of Abyssea - Grauberg
        if not player:hasKeyItem(tpz.ki.MAP_OF_ABYSSEA_GRAUBERG) then
            player:messageSpecial(ID.text.KEYITEM_OBTAINED, tpz.ki.MAP_OF_ABYSSEA_GRAUBERG)
            player:addKeyItem(tpz.ki.MAP_OF_ABYSSEA_GRAUBERG)
	...
	elseif (option == 524292 and cruor >= 100) then -- STR-DEX Boost
		player:addStatusEffectEx(tpz.effect.ABYSSEA_STR, tpz.effect.STR_BOOST_II, 10 + (tpz.abyssea.getAbyssiteTotal(player, "FURTHERANCE") * 10), 0, 0)
		player:addStatusEffectEx(tpz.effect.ABYSSEA_DEX, tpz.effect.DEX_BOOST_II, 10 + (tpz.abyssea.getAbyssiteTotal(player, "FURTHERANCE") * 10), 0, 0)
		player:delCurrency("cruor", 100)
	elseif (option == 589828 and cruor >= 100) then -- VIT-AGI Boost
		player:addStatusEffectEx(tpz.effect.ABYSSEA_VIT, tpz.effect.VIT_BOOST_II, 10 + (tpz.abyssea.getAbyssiteTotal(player, "FURTHERANCE") * 10), 0, 0)
		player:addStatusEffectEx(tpz.effect.ABYSSEA_AGI, tpz.effect.AGI_BOOST_II, 10 + (tpz.abyssea.getAbyssiteTotal(player, "FURTHERANCE") * 10), 0, 0)
		player:delCurrency("cruor", 100)
	elseif (option == 655364 and cruor >= 100) then -- INT-MND-CHR Boost
		player:addStatusEffectEx(tpz.effect.ABYSSEA_INT, tpz.effect.INT_BOOST_II, 10 + (tpz.abyssea.getAbyssiteTotal(player, "FURTHERANCE") * 10), 0, 0)
		player:addStatusEffectEx(tpz.effect.ABYSSEA_MND, tpz.effect.MND_BOOST_II, 10 + (tpz.abyssea.getAbyssiteTotal(player, "FURTHERANCE") * 10), 0, 0)
		player:addStatusEffectEx(tpz.effect.ABYSSEA_CHR, tpz.effect.CHR_BOOST_II, 10 + (tpz.abyssea.getAbyssiteTotal(player, "FURTHERANCE") * 10), 0, 0)
		player:delCurrency("cruor", 100)
