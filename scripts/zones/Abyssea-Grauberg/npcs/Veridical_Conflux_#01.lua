...
function onTrigger(player, npc)
    ...
    player:startEvent(2132, 4718592, 3932244, 6029400, 6291532, 255, 0, 1, cruor)
end

function onEventUpdate(player, csid, option)
--	printf("Aby-Grauberg veridical_conflux_01.cpp onEventUpdate CSID: [%i]  OPTION: [%i]\n", csid, option)
	local cruor = player:getCurrency("cruor")
	if (option == 131073 and cruor >= 72) then -- Veridical Conflux #02
		player:updateEvent(1, 0, 0, 6291532, 255, 0, 1, cruor) -- ?, ?, ?, flux id?, fluxesUnlocked, ?, ?, cruor
		player:delCurrency("cruor", 72)
	elseif (option == 196609 and cruor >= 84) then -- Veridical Conflux #03
		player:updateEvent(1, 0, 0, 6291532, 255, 0, 1, cruor)
