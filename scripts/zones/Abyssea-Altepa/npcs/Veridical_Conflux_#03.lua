...
function onTrigger(player, npc)
    ...
    player:startEvent(2134, 4980736, 4456488, 4718676, 5767236, 255, 2, 1, cruor)
end

function onEventUpdate(player, csid, option)
	local cruor = player:getCurrency("cruor")
	if (option == 65537) then -- Veridical Conflux #01
		player:updateEvent(1, 41, 100, 5767236, 255, 2, 1, cruor)
	...
	elseif (option == 458753 and cruor >= 68) then -- Veridical Conflux #07
		player:updateEvent(1, 41, 100, 5767236, 255, 2, 1, cruor)
		player:delCurrency("cruor", 68)
	elseif (option == 524289 and cruor >= 88) then -- Veridical Conflux #08
		player:updateEvent(1, 41, 100, 5767236, 255, 2, 1, cruor)
