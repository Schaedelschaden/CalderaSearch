...
--  printf("check.lua weaponACC: [%i]", weaponACC)
    ...
    local SDTSlash = targ:getMod(tpz.mod.SLASHRES) / 10
    local SDTPierce = targ:getMod(tpz.mod.PIERCERES) / 10
    local SDTImpact = targ:getMod(tpz.mod.IMPACTRES) / 10
    local SDTH2H = targ:getMod(tpz.mod.HTHRES) / 10
...
--  printf("check.lua Adjusted DEF: [%i]\n", DEF)
    ...
    player:PrintToPlayer(string.format("SDT - Slash: [%i%%]  Pierce: [%i%%]  Impact: [%i%%]  H2H: [%i%%]", SDTSlash, SDTPierce, SDTImpact, SDTH2H),tpz.msg.channel.SYSTEM_3)
