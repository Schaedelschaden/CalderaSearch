...
--	local targ = GetPlayerByName(target)
	...
	-- if (player:getCharVar("ServerLS_RCVD") > 0) then
...
--		player:PrintToPlayer(string.format("Gave %s a server linkpearl! They will need to zone to refresh their inventory.", target))
		player:PrintToPlayer(string.format("You just received a server linkpearl! You will need to zone to refresh your inventory."))
--		player:setCharVar("ServerLS_RCVD", 1)
