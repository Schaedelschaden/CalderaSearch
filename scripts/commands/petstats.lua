...
function onTrigger(player)
	if (player:hasPet() == true) then
		...
		local PetEnmity    = pet:getMod(tpz.mod.ENMITY)
		...
		local PetDT            = utils.clamp(pet:getMod(tpz.mod.DMG), -87.5, 87.5)
		local PetPDT           = utils.clamp(pet:getMod(tpz.mod.DMGPHYS), -87.5, 87.5)
		local PetMDT           = utils.clamp(pet:getMod(tpz.mod.DMGMAGIC), -87.5, 87.5)
		...
		player:PrintToPlayer(string.format("  Regen: [%i]  Refresh: [%i]  Regain: [%i]  Dbl Atk: [%i%%]  Tpl Atk: [%i%%]  DT: [%3.2f%%]  PDT: [%3.2f%%]  MDT: [%3.2f%%]  Crit Hit Rate: [%i%%]  SC DMG: [%i%%]", PetRegen, PetRefresh, PetRegain, PetDoubleAttack, PetTripleAttack, PetDT, PetPDT, PetMDT, PetCritHitRate, PetSCDMG),tpz.msg.channel.SYSTEM_3)
		...
		player:PrintToPlayer(string.format("You must have a pet to use that command."),tpz.msg.channel.SYSTEM_3)
	end
end
