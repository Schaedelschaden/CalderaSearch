...
function onTrigger(player, page)
    ...
    local magicMACC, magicWSMACC = calcMACC(player)
    ...
    if player:getCursorTarget() ~= nil and player:getCursorTarget():isMob() and player:hasStatusEffect(tpz.effect.FOIL) then
        EVA  = EVA + player:getMod(tpz.mod.TP_MOVE_EVASION)
        MEVA = MEVA + player:getMod(tpz.mod.TP_MOVE_EVASION)
    end
    ...
    local EnhMagicDuration  = 100 + player:getMod(tpz.mod.ENH_MAGIC_DURATION)
    ...
    local StoreTP           = player:getMod(tpz.mod.STORETP) + player:getMerit(tpz.merit.STORE_TP_EFFECT)
    ...
    local FastCast          = 100 - utils.clamp(player:getMod(tpz.mod.FASTCAST), 0, 80)
    local fastCastRecast    = 100 - (utils.clamp(player:getMod(tpz.mod.FASTCAST), 0, 80) / 2)
    ...
    local Perpetuation      = utils.clamp(player:getMod(tpz.mod.AVATAR_PERPETUATION) - player:getMod(tpz.mod.PERPETUATION_REDUCTION), 1, 15)
    local AugmentSongs      = player:getMod(tpz.mod.AUGMENT_SONGS)
    local zanshinProcRate   = player:getMod(tpz.mod.ZANSHIN)
    ...
    local Sublimation       = player:getMod(tpz.mod.SUBLIMATION_BONUS)

    local WHMCast    = 100 + player:getMod(tpz.mod.WHITE_MAGIC_CAST)
    local EnhCast    = 100 + player:getMod(tpz.mod.ENH_MAGIC_CAST_TIME)
    local BLMCast    = 100 + player:getMod(tpz.mod.BLACK_MAGIC_CAST)
    local EnfCast    = 100 + player:getMod(tpz.mod.ENFEEBLING_MAGIC_CAST)
    local DRKCast    = 100 + player:getMod(tpz.mod.DARK_MAGIC_CAST)
    local BLUCast    = 100 + player:getMod(tpz.mod.BLUE_MAGIC_CAST_TIME)
    local WHMRecast  = 100 + player:getMod(tpz.mod.WHITE_MAGIC_RECAST)
    local BLMRecast  = 100 + player:getMod(tpz.mod.BLACK_MAGIC_RECAST)
    local ElemRecast = 100 + player:getMod(tpz.mod.ELEMENTAL_MAGIC_RECAST)
    local BLURecast  = 100 + player:getMod(tpz.mod.BLUE_MAGIC_RECAST)
    ...
    if page == 0 or page == 1 or page == nil then
        ...
        player:PrintToPlayer(string.format("  ATK: [%i]  R.ATK: [%i]  Crit Hit Rate: [%i%%]  Crit Hit DMG: [%i%%]" , ATT, RATT, CritHitRate, CritHitDamage),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Accuracy: [%i]  R.Accuracy: [%i]  M.Accuracy: [%i]  M.WS.Accuracy: [%i]  Evasion: [%i]  Magic EVA: [%i]  Magic DEF: [%i]", ACC, RACC, magicMACC, magicWSMACC, EVA, MEVA, MDEF),tpz.msg.channel.SYSTEM_3)
        ...
        player:PrintToPlayer(string.format("  Parry: [%i%%]  Block: [%i%%]  Guard: [%i%%]  Counter: [%i%%]  Counter DMG: [%i]  Counter Crit Hit Rate: [%i%%]", Parry, Block, Guard, Counter, CounterDMG, CounterCHR),tpz.msg.channel.SYSTEM_3)
        ...
        player:PrintToPlayer(string.format("  Subtle Blow: [%i]  Subtle Blow II: [%i]  Zanshin Proc: [%i%%]", SubtleBlow, SubtleBlowII, zanshinProcRate),tpz.msg.channel.SYSTEM_3)
    ...
    elseif page == 2 then
        player:PrintToPlayer(string.format("MAGIC STATISTICS ----------------------------------------------------------------------------------------------------------------------------------"),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  MAB: [%i]  M.DMG: [%i]  M.Burst DMG: [%i%%]  M.Crit Rate: [%i%%]  M.Crit DMG: [%i%%]" , MATT, MDMG, MBurst, MCritRate, MCritDMG),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Sublimation: [%i%%]", Sublimation),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Spell Interrupt Down: [%i%%]  Conserve MP: [%i%%]", SID, ConserveMP),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Healing - Cure Potency: [%i%%]  Cure Potency II: [%i%%]  Cure Rec. Potency: [%i%%]", CPot, CPotII, CRPot),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Healing - Waltz Potency: [%i%%]  Waltz Rec. Potency: [%i%%]", WaltzPot, WaltzPotRecv),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("MAGIC CAST/RECAST STATISTICS ----------------------------------------------------------------------------------------------------------------------"),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Fast Cast    - Cast Time: [%i%%]  Recast Time: [%i%%]", FastCast, fastCastRecast),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Cast Time    - White: [%i%%]  Enhance: [%i%%]  Black: [%i%%]  Enfeeble: [%i%%]  Dark: [%i%%]  Blue: [%i%%]", WHMCast, EnhCast, BLMCast, EnfCast, DRKCast, BLUCast),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Recast Time - White: [%i%%]  Black: [%i%%]  Elemental: [%i%%]  Blue: [%i%%]", WHMRecast, BLMRecast, ElemRecast, BLURecast),tpz.msg.channel.SYSTEM_3)
        player:PrintToPlayer(string.format("  Duration     - Enhancing: [%i%%]", EnhMagicDuration),tpz.msg.channel.SYSTEM_3)
    end
...
function calcMACC(player)
    ...
    local MACC = player:getMod(tpz.mod.MACC) + player:getILvlMacc(tpz.slot.MAIN) + player:getMerit(tpz.merit.ELEMENTAL_MAGIC_ACCURACY)
    for i = 1, 9 do
    ...
    local magicMACC = MACC

    -- Magical Weaponskill MACC
    local weaponType = player:getWeaponSkillType(tpz.slot.MAIN)
    local rangedWeaponType = player:getWeaponSkillType(tpz.slot.RANGED)

    if player:getMainJob() == tpz.job.RNG or player:getMainJob() == tpz.job.COR then
        MACC = player:getMod(tpz.mod.MACC) + player:getSkillLevel(rangedWeaponType) + player:getILvlMacc(tpz.slot.MAIN) + player:getILvlSkill(tpz.slot.RANGED)
    else
        MACC = player:getMod(tpz.mod.MACC) + player:getSkillLevel(weaponType) + player:getILvlMacc(tpz.slot.MAIN) + player:getILvlSkill(tpz.slot.MAIN)
    end

    local magicWSMACC = MACC

    return magicMACC, magicWSMACC
end