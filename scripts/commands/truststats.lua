...
function onTrigger(player)
	...
	if (player:getCursorTarget() ~= nil) then
		...
		if (trust:getObjType() == tpz.objType.TRUST) then
			...
			player:PrintToPlayer(string.format("  HP: [%i/%i]  ATT: [%i]  MATT: [%i]  DEF: [%i]  MDEF: [%i]  ACC: [%i]  MACC: [%i]  RACC: [%i]  EVA: [%i]  MEVA: [%i]", trustCurrentHP, trustMaxHP, trustATT, trustMATT, trustDEF, trustMDEF, trustACC, trustMACC, trustRACC, trustEVA, trustMEVA),tpz.msg.channel.SYSTEM_3)
