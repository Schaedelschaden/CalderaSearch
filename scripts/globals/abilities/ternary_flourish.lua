...
function onAbilityCheck(player, target, ability)
    if (player:hasStatusEffect(tpz.effect.FINISHING_MOVE_1) or 
		player:hasStatusEffect(tpz.effect.FINISHING_MOVE_2)) then
	...
	elseif (player:hasStatusEffect(tpz.effect.FINISHING_MOVE_3) or
...
function onUseAbility(player, target, ability)
    ...
    player:addStatusEffect(tpz.effect.TERNARY_FLOURISH, 1, 0, 60)
end