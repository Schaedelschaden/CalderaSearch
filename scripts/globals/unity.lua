...
function tpz.unity.onTrigger(player, npc)
    ...
    -- local remainingLimit = tpz.settings.WEEKLY_EXCHANGE_LIMIT - player:getCharVar("weekly_accolades_spent")
    ...
    player:startEvent(zoneEventIds[zoneId][4], 0, 1, 99999, 0, 0, 0, 0, 0)
end
...
function tpz.unity.onEventUpdate(player, csid, option)
    ...
    elseif category == 4 then
        ...
        if npcUtil.giveItem(player, { {itemId, qty} }) then
            ...
            if tpz.settings.ENABLE_EXCHANGE_LIMIT == 1 then
