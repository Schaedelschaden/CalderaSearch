...
function onSpellCast(caster, target, spell)
	...
	if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
		power = (skill / 18) + (GeomancyPotency(caster) * 2)
	else
		power = (skill / 18)
	end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 9, 3, duration, tpz.effect.GEO_EVASION_DOWN, power, tpz.auraTarget.ENEMIES, tpz.effectFlag.AURA)
