...
function onSpellCast(caster, target, spell)
	...
	if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
		power = (skill / 60.4027) + (GeomancyPotency(caster) / 2)
	else
		power = (skill / 60.4027)
	end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 11, 3, duration, tpz.effect.GEO_SLOW, power, tpz.auraTarget.ENEMIES, tpz.effectFlag.AURA)
