...
function onSpellCast(caster, target, spell)
	...
	if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
		power = (skill / 13.8461) + (GeomancyPotency(caster) * 6)
	else
		power = (skill / 13.8461)
	end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 6, 3, duration, tpz.effect.GEO_MAGIC_EVASION_BOOST, power, tpz.auraTarget.ALLIES, tpz.effectFlag.AURA)
