...
function onSpellCast(caster,target,spell)
    ...
    if (target:hasStatusEffect(tpz.effect.DEX_DOWN) or caster:hasStatusEffect(tpz.effect.DEX_BOOST)) then
        ...
        if (resist <= 0.125) then
            ...
            caster:addStatusEffect(tpz.effect.DEX_BOOST, Absorb, ABSORB_SPELL_TICK, 90) -- caster gains DEX
            target:addStatusEffect(tpz.effect.DEX_DOWN, Absorb, ABSORB_SPELL_TICK, 90) -- target loses DEX
        end
    end
