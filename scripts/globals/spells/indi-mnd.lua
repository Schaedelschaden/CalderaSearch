...
function onSpellCast(caster, target, spell)
	...
	if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
		power = (skill / 36) + (GeomancyPotency(caster) * 2)
	else
		power = (skill / 36)
	end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 5, 3, duration, tpz.effect.GEO_MND_BOOST, power, tpz.auraTarget.ALLIES, tpz.effectFlag.AURA)
