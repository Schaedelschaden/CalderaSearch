...
function onSpellCast(caster, target, spell)
	...
	if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
		power = (skill / 36) + (GeomancyPotency(caster) * 4.6)
	else
		power = (skill / 36)
	end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 13, 3, duration, tpz.effect.GEO_ATTACK_DOWN, power, tpz.auraTarget.ENEMIES, tpz.effectFlag.AURA)
