...
function onSpellCast(caster, target, spell)
	...
	if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
		power = (skill / 45) + (GeomancyPotency(caster) * 4)
	else
		power = (skill / 45)
	end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 8, 3, duration, tpz.effect.GEO_MAGIC_ATK_DOWN, power, tpz.auraTarget.ENEMIES, tpz.effectFlag.AURA)
