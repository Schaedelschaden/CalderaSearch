...
function onSpellCast(caster, target, spell)
	...
	if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
		power = (skill / 22.6131) + (GeomancyPotency(caster) * 4.6)
	else
		power = (skill / 22.6131)
	end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 3, 3, duration, tpz.effect.GEO_DEFENSE_BOOST, power, tpz.auraTarget.ALLIES, tpz.effectFlag.AURA)
