...
function onSpellCast(caster, target, spell)
	...
	if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
		power = (skill / 45.2261) + (GeomancyPotency(caster) * 1.1)
	else
		power = (skill / 45.2261)
	end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 10, 3, duration, tpz.effect.GEO_WEIGHT, power, tpz.auraTarget.ENEMIES, tpz.effectFlag.AURA)
