...
function onSpellCast(caster, target, spell)
    ...
    if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
        power = (skill / 30) + (GeomancyPotency(caster) * 2)
    else
        power = (skill / 30)
    end

    if caster:getObjType() == tpz.objType.TRUST and caster:hasStatusEffect(tpz.effect.ENTRUST) then
        power = math.floor(utils.clamp(caster:getMainLvl(), 0, 99) / 3.3)
    end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 6, 3, duration, tpz.effect.GEO_REGEN, power, tpz.auraTarget.ALLIES, tpz.effectFlag.AURA)
