...
function onSpellCast(caster, target, spell)
    ...
    if caster:getObjType() == tpz.objType.TRUST then
        power = math.floor(utils.clamp(caster:getMainLvl(), 0, 99) / 1.8)
    end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 7, 3, duration, tpz.effect.GEO_MAGIC_ACC_BOOST, power, tpz.auraTarget.ALLIES, tpz.effectFlag.AURA)
