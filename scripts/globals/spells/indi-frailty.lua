...
function onSpellCast(caster, target, spell)
    ...
    if not (caster:hasStatusEffect(tpz.effect.ENTRUST)) then
        power = (skill / 60.8108) + (GeomancyPotency(caster) * 2.7)
    else
        power = (skill / 60.8108)
    end

    if caster:getObjType() == tpz.objType.TRUST then
        power = math.floor(utils.clamp(caster:getMainLvl(), 0, 99) / 7.92)
    end
    ...
    target:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 10, 3, duration, tpz.effect.GEO_DEFENSE_DOWN, power, tpz.auraTarget.ENEMIES, tpz.effectFlag.AURA)
