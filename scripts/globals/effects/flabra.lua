...
function onEffectGain(target, effect)
	local RuneCounter = target:countEffect(tpz.effect.FLABRA)
	...
	target:addMod(tpz.mod.PETRIFYRES, effect:getPower() / 2)
	target:addMod(tpz.mod.SLOWRES, effect:getPower() / 2)
end
...
function onEffectLose(target, effect)
	...
	target:delMod(tpz.mod.PETRIFYRES, effect:getPower() / 2)
	target:delMod(tpz.mod.SLOWRES, effect:getPower() / 2)
end