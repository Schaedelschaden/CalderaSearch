...
function onUseWeaponSkill(player, target, wsID, tp, primary, action, taChar)
	...
	local chance = utils.clamp(tp / 30, 50, 100) -- 50% to 100% chance to stun based on TP

    if (damage > 0 and math.random(1, 100) < chance and target:hasStatusEffect(tpz.effect.STUN) == false) then
