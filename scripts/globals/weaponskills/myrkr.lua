...
function onUseWeaponSkill(player, target, wsID, tp, primary, action, taChar)
    ...
    -- Check for up to 3 status effects from the removable list
    for i = 1, #removables do
        if
            player:hasStatusEffect(removables[i]) and
            count < maxRemoved
        then
            player:delStatusEffect(removables[i])

            count = count + 1
        end
    end

    -- Check for up to 3 Erasable status effects if removables are not present
    for i = 1, (maxRemoved - count) do
        if count < maxRemoved then
            player:eraseStatusEffect()
