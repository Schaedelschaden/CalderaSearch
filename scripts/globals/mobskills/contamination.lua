---------------------------------------------
-- Contamination
-- Family: Cockatrice NM's
-- Description: Transfers all status ailments to the targets
-- Type: Enfeebling
...
function onMobWeaponSkill(target, mob, skill)
    ...
    local dmg = utils.takeShadows(target, 1, 2) --removes 2 shadows
    --if removed more shadows than were up or there weren't any
    if (dmg > 0) then
        for i, effect in ipairs(removables) do
            if (mob:hasStatusEffect(effect)) then
				mob:setLocalVar("CONTAMINATION_"..i, effect)
				mob:setLocalVar("CONTAMINATION_TOTAL", mob:getLocalVar("CONTAMINATION_TOTAL") + 1)
